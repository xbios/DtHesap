...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <h2 class="text-2xl font-bold text-gray-900">Cari Düzenle: {{ $cari->unvan }}</h2>
        <a href="{{ route('caris.index') }}" class="text-sm text-gray-600 hover:text-gray-900">← Geri Dön</a>
    </div>

    <div class="bg-white shadow sm:rounded-lg overflow-hidden">
        <form action="{{ route('caris.update', $cari) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="kod" id="kod" value="{{ old('kod', $cari->kod) }}" required
                ...
                <div>
                    ...
                    <select name="tip" id="tip" required
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        <option value="musteri" {{ old('tip', $cari->tip) == 'musteri' ? 'selected' : '' }}>Müşteri</option>
                        <option value="tedarikci" {{ old('tip', $cari->tip) == 'tedarikci' ? 'selected' : '' }}>Tedarikçi</option>
                        <option value="her_ikisi" {{ old('tip', $cari->tip) == 'her_ikisi' ? 'selected' : '' }}>Her İkisi</option>
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="unvan" id="unvan" value="{{ old('unvan', $cari->unvan) }}" required
                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <div>
                    ...
                    <input type="text" name="vergi_dairesi" id="vergi_dairesi" value="{{ old('vergi_dairesi', $cari->vergi_dairesi) }}"
                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="vergi_no" id="vergi_no" value="{{ old('vergi_no', $cari->vergi_no) }}"
                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="telefon" id="telefon" value="{{ old('telefon', $cari->telefon) }}"
                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                </div>
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $cari->email) }}"
                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="adres" id="adres" rows="3"
                              class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('adres', $cari->adres) }}</textarea>
                </div>
            </div>
