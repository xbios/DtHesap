...
@section('content')
<div class="max-w-7xl mx-auto space-y-6" x-data="faturaForm()">
    <div class="flex items-center justify-between">
        <h2 class="text-2xl font-bold text-gray-900">Yeni Fatura Oluştur</h2>
        <a href="{{ route('faturas.index') }}" class="text-sm text-gray-600 hover:text-gray-900">← Geri Dön</a>
    </div>

    <form action="{{ route('faturas.store') }}" method="POST" @submit.prevent="submitForm">
        @csrf
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div class="lg:col-span-3 space-y-6">
                <div class="bg-white shadow sm:rounded-lg p-6">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select name="cari_id" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                @foreach($cariler as $cari)
                                    <option value="{{ $cari->id }}">{{ $cari->unvan }} ({{ $cari->kod }})</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="fatura_no" required value="{{ old('fatura_no') }}"
                        ...
                        <div>
                            ...
                            <input type="date" name="vade_tarih" value="{{ old('vade_tarih') }}"
                                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="fatura_tip" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <option value="satis" {{ old('fatura_tip') == 'satis' ? 'selected' : '' }}>Satış Faturası</option>
                                <option value="alis" {{ old('fatura_tip') == 'alis' ? 'selected' : '' }}>Alış Faturası</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white shadow sm:rounded-lg p-6">
                    ...
                    <textarea name="aciklama" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500"></textarea>
                </div>
            </div>
